...
def _translate(image, horizontal, vertical):#horizontal=(0, 40), vertical=(0, 10)):
#     '''
...
#     return cv2.warpAffine(image, M, (cols, rows)), x_shift, y_shift
    return random_shift(image, horizontal, vertical, row_axis=0, col_axis=1, channel_axis=2, fill_mode='nearest', cval=0)

def _rotation(image, rg, row_axis=0, col_axis=1, channel_axis=2, fill_mode='nearest', cval=0.):
    '''
    Randomly rotates input image in interval [-angle, angle]
    Parser to keras rotation method:
    ...
    return random_rotation(image, rg, row_axis, col_axis, channel_axis, fill_mode, cval=0)
...
def _scale(image, zoom_range, row_axis=0, col_axis=1, channel_axis=2, fill_mode='nearest', cval=0.):
    """
    Parser to keras rotation method:
    ...
    """
    return random_zoom(image, zoom_range, row_axis, col_axis, channel_axis, fill_mode, cval=0.)
    ...
    def generate(self,
        ...
        Arguments:
            ...
            translate (tuple, optional): `False` or a tuple, with the first two elements tuples containing
                ...
                The respective box coordinates are adjusted accordingly.
            random_rotate (tuple, optional):  `False` or an integer in [0,180] that represents the angle of maximum rotation.
        ...
        while True:
            ...
            for i in range(len(batch_X)):
                ...
                if translate:
                    batch_X[i] = _translate(batch_X[i], translate[0], translate[1])
#                     p = np.random.uniform(0, 1)
#                     if p >= (1 - translate[2]):
...
#                                         after_area >= include_thresh * before_area]  # Especially for the case `include_thresh == 1` we want the ">=" sign, otherwise no boxes would be left at all
                if random_rotation:
                    ...
                    batch_X[i] = _rotation(batch_X[i], random_rotation)
                if scale:
                    batch_X[i] = _scale(batch_X[i], scale)
#                     p = np.random.uniform(0, 1)
#                     if p >= (1 - scale[1]):
